...
def test_generate_report_returns_dataset_insights(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert body["charts_available"] == ["histogram", "box", "bar", "scatter"]
    assert any("duplicate rows" in recommendation.lower() for recommendation in body["recommendations"])


def test_generate_report_with_ai_insights(monkeypatch: pytest.MonkeyPatch) -> None:
    """A populated dataset returns structured insights generated by AI."""
    csv_contents = (
        "region,sales,margin,notes\n"
        "North,100,0.20,\n"
        "North,100,0.20,\n"
    ...
    assert response.status_code == 200
    assert body["key_insights"] == ["AI Key Insight"]
    assert body["recommendations"] == ["AI Recommendation"]
...
@pytest.mark.parametrize(
    ...
    ids=["upload", "chart", "dashboard", "analytics", "dataset", "generate-report"],
)
